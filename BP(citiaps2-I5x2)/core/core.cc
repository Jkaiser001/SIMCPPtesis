    ...
    //cout<<"Entro run3"<<endl;
     ...
     for(int i=0; i< npag; i++)
     {
...
#ifdef MIDE_ESTADISTICAS
           estadisticas->hit_L1( (*pthread)->pid );
           estadisticas->lecturas((*pthread)->pid/NCORE,0);
#endif
           L1->update( str );// me falta este tiempo
           if (t_cpu!=0.0) (*pthread)->phold( t_cpu );
        }
        else if ( L2->hit( str ) )
       {
            ...
            estadisticas->fallaL1( (*pthread)->pid  );
            estadisticas->hit_L2( (*pthread)->pid );  
            estadisticas->escrituras((*pthread)->pid/NCORE,0);
            estadisticas->lecturas((*pthread)->pid/NCORE,1);
        #endif
          //cout<<"hit_L2, "<<cpid<<" - "<<id_core<<endl;
          L2->update( str );//me falta este tiempo
          L1->insert( str );
        ...
        else if (L3->hit( str ))
        {
            ...
            estadisticas->escrituras((*pthread)->pid/NCORE,0);
            estadisticas->escrituras((*pthread)->pid/NCORE,1);
            estadisticas->lecturas((*pthread)->pid/NCORE,2);
          #endif
        ...
        else
        {
            ...
            estadisticas->escrituras((*pthread)->pid/NCORE,0);
            estadisticas->escrituras((*pthread)->pid/NCORE,1);
            estadisticas->escrituras((*pthread)->pid/NCORE,2);
            estadisticas->lecturas((*pthread)->pid/NCORE,3);
          #endif
          L3->insert( str );
          L2->insert( str );
          L1->insert( str );
